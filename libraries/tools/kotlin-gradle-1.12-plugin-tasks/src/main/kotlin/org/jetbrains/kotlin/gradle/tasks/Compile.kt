...
public class DaemonKotlinCompiler<T : CommonCompilerArguments>(
        val daemonWorkingDir: File,
        val delegate: Compiler<KotlinCompileSpec<T>>,
        val compilerDaemonFactory: CompilerDaemonFactory)
: Compiler<KotlinCompileSpec<T>> {

    fun toDaemonOptions(spec: KotlinCompileSpec<T>): DaemonForkOptions {
        val forkOptions = spec.kotlinCompileLaunchingOptions.forkOptions
    ...
    override fun execute(spec: KotlinCompileSpec<T>): WorkResult {
        val daemonForkOptions = toDaemonOptions(spec)
        val daemon = compilerDaemonFactory.getDaemon(daemonWorkingDir, daemonForkOptions)
        val result = daemon.execute(delegate, spec)
        if (result.isSuccess()) {
            return result
        }
...
//            if (compilerDaemonManager != null) return DaemonKotlinCompiler(project.getProjectDir(), kotlinCompiler, compilerDaemonManager)
