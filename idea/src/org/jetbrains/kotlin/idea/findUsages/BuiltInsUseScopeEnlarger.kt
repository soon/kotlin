...
public class BuiltInsUseScopeEnlarger : UseScopeEnlarger() {
    private val builtInDirUrls = BuiltInsReferenceResolver.getBuiltInsDirUrls().map { VfsUtilCore.toIdeaUrl(it.toString()) }

    override fun getAdditionalUseScope(element: PsiElement): SearchScope? {
        val url = element.getContainingFile()?.getVirtualFile()?.getUrl()
        if (url != null && VfsUtilCore.isUnder(url, builtInDirUrls)) {
